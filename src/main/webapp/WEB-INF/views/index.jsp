...
<body>
	...
	<article class="idx_viewpoint">
					...
					<c:when test="${not empty TopPic}">
						<c:forEach items="${TopPic}" var="item" varStatus="vs">
							<c:if test="${vs.index==0 }">
								<li class="swiper-slide swiper-slide-active" data-swiper-slide-index="0" style="width: 320px;"><a href="#"><img src="${item.pic }" style="opacity: 1; transition: opacity 0.3s;"></a></li>
							</c:if>
							<c:if test="${vs.index==1 }">
								<li class="swiper-slide swiper-slide-next" data-swiper-slide-index="${vs.index }" style="width: 320px;"><a href="#"><img src="${item.pic }" style="opacity: 1; transition: opacity 0.3s;"></a></li>
							</c:if>
							<c:if test="${vs.index>1 }">
								<li class="swiper-slide" data-swiper-slide-index="${vs.index }" style="width: 320px;"><a href="#"><img src="${item.pic }" style="opacity: 1; transition: opacity 0.3s;"></a></li>
							</c:if>
						</c:forEach>
					</c:when>
				</c:choose>
		...
		<nav style="margin-bottom: 0;">
			<ul>
				...
				<!-- li><a href="#"><div class="cat_icon">
							<img src="<%=path%>/resources/images/1179liwugou_wap.png"
								style="opacity: 1; transition: opacity 0.3s;">
						</div>
						<p class="cat_name">送礼物</p></a></li-->
		...
		<c:if test="${not empty bighit}">
			<div id="bighitdiv" class="yly_tjian">
				<section class="plc_idx_module">
						...
						<span class="line" style="background-color:#e73028"></span>
							&nbsp<img src="${ctx}/resources/images/shangcheng.png" class="imgs" />&nbsp
							<span style="color:#e73028">商城推荐</span>
							<span class="line" style="background-color:#e73028"></span>
						</h4>
				</section>
				<c:forEach items="${bighit}" var="item" varStatus="vs">
					<div class="yl_bc86" onclick="OpenProduct('${item.gId}');">
						<div class="tjian_pro_bighit">
							...
							<div class="pro_name_bighit yly_color8 yly_font_size3" style="margin-top:5px">${item.gName}</div>
							<div class="pro_pric">
								<div class="prics">
									<span class="yl_bc30 yly_color12 yly_font_size8">￥${item.price}</span>
								</div>
								<div class="prics yl_bc29">
									<span class="yl_bc31 yly_color3 yly_font_size3" style="overflow:hidden;text-overflow:ellipsis"><nobr>${item.sales+50}人付款</nobr></span>
								</div>
							</div>
						</div>
					</div>
		...
		<div id="footer">
			<ul>
				...
				<li><a href="<%=path%>/cart/" class="nav3">
	...
	<script type="text/javascript"
		src="<%=path%>/resources/js/my_index.js"></script>
		...
		$.post("<%=path%>/user/ajaxLogin.api", {
				...
				if (status != "0") {
					if(isWeiXin()){
						...
						$.post(ctx + "/WXLoginByOpenId", {phone:"",OpenId:OpenId},function(result) {
...
$(function() {
	if(isWeiXin()){
		...
		if(subscribe!="" && subscribe!=null && subscribe==1){
			$('#guanzhu').html('<a href="http://mp.weixin.qq.com/s?__biz=MzI0NTU0MTUzOA==&tempkey=Czm706DrhzRjyqXytgDFazKFwuUXuj1zzdA5dCkTz1BAHlrZ3i0XLuNyucqN5dfuKnXoYqJMUHUnDeaoRrzGd5peNb79Ieb13IbDRkhoZCopCUwAJ%2FiFxwAtUCby2nMzTZ%2FyVzq%2BCUaxtv7iHW7D4g%3D%3D&#rd">关注</a>');
		}
	}
...
$(window).scroll(function(){ 
    ...
    if($(document).height() <= totalheight){ 
        if(stop==true){ 
			stop=false; 
            $.post(ctx + '/superaddhit', {
				"current" : $('#bighitCount').val(),
			}, function(result) {
				...
				var temp = parseInt($('#bighitCount').val())+json.length;
				...
				for(var i=0;i<json.length;i++){
					var hitdiv = "<div class=\"yl_bc86\" onclick=\"OpenProduct('"+json[i]['gId']+"');\">"
								...
								+"<span class=\"yl_bc31 yly_color3 yly_font_size3\">"+json[i]['sales']+"人付款</span>"
								+"</div></div></div></div>";
					$('#bighitdiv').append(hitdiv);
				}
				stop=true;
			});
        } 
